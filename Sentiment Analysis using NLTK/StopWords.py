...
print "Maximum Entropy classifier accuracy of raw data without stop words"
...
def word_features(words):
    return dict([(word, True) for word in words if word not in stop_set])
...
posCutoff = len(posWords) * 90 / 100
negCutoff = len(negWords) * 90 / 100
...
classifier = MaxentClassifier.train(Train_set, algorithm=algo, max_iter=5)
